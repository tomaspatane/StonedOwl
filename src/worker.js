...
function articlesBelongTogether(a, b) {
  ...
  if (score.shared >= 3 && score.overlap >= 0.40) {
    return true;
  }

  if (score.shared >= 2 && score.jaccard >= 0.28) {
    return true;
  }
...
function clusterArticles(articles = []) {
  ...
  const tokenSets = sample.map(article =>
    titleTokens(article.title)
  );
  ...
  return [...components.values()]
    ...
    .map(indices => {
      const groupedArticles = indices.map(
        index => sample[index]
      );
...
function actorKey(value = '') {
  ...
  if (/\bsube\b/.test(normalized)) {
    return 'sube';
  }

  if (/\bindec\b/.test(normalized)) {
    return 'indec';
  }
...
function liveInterpretation(
  ...
  if (dominant) {
    const count =
      dominant.sourceCount ||
      dominant.articleCount;

    headline =
      `La cobertura recuperada se concentra en una historia repetida por ${count} ` +
      `fuente${count === 1 ? '' : 's'} de la muestra.`;
  }
...
async function monitorInterpretation(
  db,
  monitor
) {
  const latestBase =
