...
async function fetchBingNews(q, scope, span) {
  ...
  const response = await fetchWithTimeout(url.toString(), {
    headers: {
      ...
      'user-agent': 'Mozilla/5.0 (compatible; StonedOwl/0.8)'
    }
  });
  const xml = await response.text();
  if (!response.ok) throw new Error(`HTTP ${response.status}: ${xml.slice(0, 220)}`);
...
async function fetchGdelt(q, scope, span) {
  ...
  if (!response.ok) throw new Error(`HTTP ${response.status}: ${raw.slice(0, 220)}`);

  let data;
  try {
    data = JSON.parse(raw);
  } catch {
    throw new Error(`respuesta no JSON: ${raw.slice(0, 220)}`);
  }
...
async function runSources(q, scope, span) {
  ...
  return {
    ...
    diagnostics: {
      bingNews: diagnostic(bing),
      gdelt: diagnostic(gdelt),
      googleNews: diagnostic(google)
    }
  };
}
...
async function captureMonitor(db, monitor) {
  ...
  return { monitorId: monitor.id, capturedAt, articleCount: articles.length, sourceCount };
}
...
async function handleRadar(db) {
  ...
  for (const monitor of monitors) {
    const { results: snapshots } = await db.prepare(`
      SELECT captured_at, article_count, source_count, active_provider_count
...
async function handleHealth(db) {
  ...
  try {
    ...
    checks.internet = { ok: false, error: String(error?.message || error) };
  }
...
async function handleNews(request) {
  ...
  const scope = url.searchParams.get('scope') || 'argentina';
  const requestedSpan = url.searchParams.get('span');
  const span = SPAN_MAP[requestedSpan] ? requestedSpan : '1w';
  ...
  if (!['argentina', 'world'].includes(scope)) return json({ error: 'Alcance inválido.' }, 400);
...
export default {
  async fetch(request, env) {
    try {
      ...
      if (url.pathname === '/api/radar') return handleRadar(env.DB);
      if (url.pathname === '/api/monitor') return handleMonitor(request, env.DB);
      return env.ASSETS.fetch(request);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      console.error(JSON.stringify({ message: 'request failed', error: message, path: new URL(request.url).pathname }));
      return json({ error: 'Error interno de StonedOwl.' }, 500);
    }
  },
